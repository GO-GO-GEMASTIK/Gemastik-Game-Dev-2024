...
@onready var node = $".."
@onready var box_kaos_kaki = %BoxKaosKaki
...
func _input(event):
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT and not event.pressed:
			selected = false
			box_kaos_kaki.animation = "default"
			var shortest_dist = 100
			for child in rest_nodes:
				var distance = global_position.distance_to(child.global_position)
				if distance < shortest_dist:
					if child.is_in_group("box_kaos_kaki"):
						queue_free()
						node.increment_count()
						correct.play()
					elif child.is_in_group("box_baju_berwarna") or child.is_in_group("box_baju_putih"):
						wrong.play()

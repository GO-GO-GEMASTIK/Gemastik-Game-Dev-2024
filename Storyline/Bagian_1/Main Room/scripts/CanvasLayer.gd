...
func _input(event):
	if !GameStateManager.get_string_state("TutorialDone"):
		#if event.is_action_pressed("jump") and tutor_2_showed and !passed_2:
			#tutor_3_showed = true
			#hide_show_tutor(tutor_3)
			#passed_2 = true
		if event.is_action_pressed("talk") and tutor_2_showed and !passed_3:
			tutor_4_showed = true
...
func hide_show_tutor(tutor):
	...
	if tutor == tutor_4:
		tutorial_player.play("show_tutorial_auto_close")
	else:
		tutorial_player.play("show_tutorial")
